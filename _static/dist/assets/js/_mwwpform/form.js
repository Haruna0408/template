...
function checkTel() {
  if ($("#tel").val() == "") {
    ...
    $(".js-errorTel").text(
      "電話番号は0から始まる半角数字のみ、10桁または11桁で入力してください。"
    );
    return 1;
  } else {
    $(".js-errorTel").text("");
...
$(function () {
  ...
  $("#confirm").on("click", function () {
    ...
    if (err_cnt > 0) {
      $(".js-errorTop").html(
        "入力内容に不備がございます。<br> お手数ですが、メッセージの表示された項目をご確認の上、再度ご入力ください。"
      );
...
//フォーム表示制御
//入力画面では確認項目完了項目を非表示
if ($(".p-contactInput").length) {
  $(".p-contactForm__confirm").css("display", "none");
  $(".p-contactForm__complete").css("display", "none");
}
//確認画面では入力項目完了項目を非表示
if ($(".p-contactConfirm").length) {
  $(".p-contactForm__complete").css("display", "none");
}
//完了画面では入力項目確認項目を非表示
if ($(".p-contactComplete").length) {
  $(".p-contactForm__confirm").css("display", "none");
}
